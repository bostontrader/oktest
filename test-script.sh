...
# 2.2 Since we are going to use BTC and LTC in our subsequent transactions, we must define them as currencies in Bookwerx. We have already done this for the OKCatbox books, but we are using the same currencies for the user's books and we must define them separately there.
CURRENCY_BTC="$(curl -d "apikey=$APIKEY&rarity=0&symbol=BTC&title=Bitcoin" $BW_SERVER_URL/currencies | jq .LastInsertId)"
CURRENCY_LTC="$(curl -d "apikey=$APIKEY&rarity=0&symbol=LTC&title=Litecoin" $BW_SERVER_URL/currencies | jq .LastInsertId)"

# 2.3 Establish some necessary bookkeeping accounts for the user.  Notice that several of the accounts have identical titles.  They are differentiated according to their currencies.
...
ACCT_SPOT_AVAIL_BTC="$(curl -d "apikey=$APIKEY&rarity=0&currency_id=$CURRENCY_BTC&title=OKEx Spot- Available" $BW_SERVER_URL/accounts | jq .LastInsertId)"
ACCT_SPOT_AVAIL_LTC="$(curl -d "apikey=$APIKEY&rarity=0&currency_id=$CURRENCY_LTC&title=OKEx Spot- Available" $BW_SERVER_URL/accounts | jq .LastInsertId)"
ACCT_SPOT_HOLD_BTC="$(curl -d "apikey=$APIKEY&rarity=0&currency_id=$CURRENCY_BTC&title=OKEx Spot- Hold" $BW_SERVER_URL/accounts | jq .LastInsertId)"
# ACCT_SPOT_HOLD_LTC="$(curl -d "apikey=$APIKEY&rarity=0&currency_id=$CURRENCY_LTC&title=OKEx Spot- Hold" $BW_SERVER_URL/accounts | jq .LastInsertId)"
...
# 2.4 Establish some necessary categories
...
# 2.5 Now tag these accounts with suitable categories.  Just do it, we don't care about saving any return values.
...
curl -d "apikey=$APIKEY&account_id=$ACCT_SPOT_AVAIL_BTC&category_id=$CAT_ASSETS" $BW_SERVER_URL/acctcats
curl -d "apikey=$APIKEY&account_id=$ACCT_SPOT_AVAIL_LTC&category_id=$CAT_ASSETS" $BW_SERVER_URL/acctcats
curl -d "apikey=$APIKEY&account_id=$ACCT_SPOT_HOLD_BTC&category_id=$CAT_ASSETS" $BW_SERVER_URL/acctcats
...
curl -d "apikey=$APIKEY&account_id=$ACCT_SPOT_AVAIL_BTC&category_id=$CAT_SPOT_AVAILABLE" $BW_SERVER_URL/acctcats
curl -d "apikey=$APIKEY&account_id=$ACCT_SPOT_AVAIL_LTC&category_id=$CAT_SPOT_AVAILABLE" $BW_SERVER_URL/acctcats
...
# 4. Start making some transactions.
...
# 4.2 Simulate the deposit of BTC into the funding account.  This is a tedious and difficult issue for a variety of reasons.  Therefore, at this point, we will...

# 4.2.1 ... use this convenience endpoint from the OKCatbox where we can easily assert a deposit. This OKCatbox endpoint is a convenience to make it easy to make deposits.  The real OKEx server doesn't manage deposits via the API.
...
# 4.2.2. Let's use okconnect to compare the user's balances in Bookwerx with the corresponding balances in the OKCatbox.  We should detect a discrepancy because the OKCatbox has a deposit,  but we haven't yet made a matching transaction on the user's books.
...
# 4.2.3 Now create the bookwerx transaction on our user's books.
